...
{
	if constexpr (N > 1){
      for (int i{}; i<std::size(container); ++i){
        fill_multi_array<N-1>(*(std::next(std::begin(container), i)), std::next(multi_array, i)->begin());
      }
	} else {
	  std::copy(container.begin(), container.end(), multi_array);
	}
}
